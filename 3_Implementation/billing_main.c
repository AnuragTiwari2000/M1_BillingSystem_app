/**
 * @file billing_main.c
 * @author Anurag Tiwari (anuragrkt46gmail.com)
 * @brief Project on Customer Billing System
 * @version 0.1
 * @date 2022-02-11
 * 
...
char customer[20];
    ...
    switch(select_option){
        ...
        for(int i=0;i<n;i++){
            ...
            printf("please enter brand %d",i+1);
        ...
        generateBillHeader(pur.customername,pur.date);
            ...
            case 2:
            fp = fopen("billing.dat","r");
            printf("\n saved bills\n");
            while(fread(&purchases,sizeof(struct purchase),1,fp))
            {float total=0;
                generateBillHeader(purchases.customername,purchases.date);
            ...
            {float totalBill=0;
               if(!strcmp(pur.customername,customer)){
                generateBillHeader(pur.customername,pur.date);
                for(int i=0;i<pur.numberOfProducts;i++){
                    generateBill(pur.prd[i].productName,pur.prd[i].brand,pur.prd[i].quantity,pur.prd[i].rate);
                    totalBill+=pur.prd[i].quantity * pur.prd[i].rate;
            ...
            if(!invoiceFound){
                printf("Sorry invoice for %s doesnt exist",customer);
            }
            fclose(fp);
            break;
